...
class Command(BaseCommand):
    help = (
        'Sends an e-mail reminder to users registered more than N days that are '
        'not enrolled into any courses yet'
    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Fallback if courses_joined relation is not present on User
            users = User.objects.filter(date_joined__date__lte=date_joined)
